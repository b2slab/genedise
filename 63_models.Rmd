  ...
  function(vars) {
    ...
    plyr::l_ply(
      ...
      function(hypothesis) {
        ...
        if (vars$input_type %in% hypothesis$exclude_input) return()
        
        # adjust model
        mod <- glm(
          as.formula(paste0(vars$metric, " ~ ", hypothesis$formula)), 
          data = df_current, 
          family = model_fam)
        
        mod_dir <- paste0(config$dir_models, "/", vars$input_type, "/", hypothesis$dir, "/")
        dir.create(path = mod_dir, recursive = TRUE)
        ...
        # in latex format
        tex_lines <- stargazer::stargazer(
          mod, ci = TRUE, ci.level = 0.95, 
          single.row = TRUE, omit.table.layout = "mdl", 
          title = paste0(model_fam, " model for ", vars$metric, 
                         " using the ", vars$input_type, " input"))
        writeLines(tex_lines, con = paste0(mod_dir, "/", file_prefix, "_model.tex"))
