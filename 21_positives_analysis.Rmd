...
write.csv(df_interdisease, 
          file = paste0(config$dir_topology, "/interdiseases_properties.csv"), 
          row.names = FALSE)
...
df_modularity <- plyr::ddply(
  ...
  function(df_dis) {
    # browser()
    disease <- df_dis$disease[1] %>% as.character
    ...
    data.frame(
      disease = disease, 
      n_drug = length(unique(genes_dis)), 
...
# save the non-random modularity values
df_modularity_save <- filter(df_modularity, !random) %>% select(-random)
write.csv(df_modularity_save, 
          file = paste0(config$dir_topology, "/modularity_diseases.csv"), 
          row.names = FALSE)
