...
df_nets_avg <- mutate(
  ...
  input_type = as.factor(input_type)) %>% 
...
list_hypotheses <- list(
  "additive" = list(
    dir = "additive", 
    plot = TRUE, 
    formula = "method + cv_scheme + disease", 
    contrasts = TRUE), 
  "interaction:cv_scheme" = list(
    dir = "interaction:cv_scheme", 
    plot = FALSE, 
    formula = "method + method:cv_scheme + disease", 
    contrasts = FALSE, 
    ...
    plyr::llply(
      ...
      function(hypothesis) {
        ...
        df_out_cast <- data.table::dcast(
          data.table::setDT(df_out), metric + method ~ cv_scheme, 
...
```{r, eval=TRUE}
local({
  filt_mod <- "input_type == 'genetic' & cv_scheme == 'block' & network == 'string'"
  dir_mod <- paste0(config$dir_models, "/genetic/reduced")
  dir.create(dir_mod)
  input <- "genetic"
  ...
  function(df) {
    ...
    gg <- ggplot(df_plot_main, aes(x = method, y = value)) + 
      ...
      ggtitle(paste0("Performance using ", input, "-related data as input and the ", network, " network"), 
              subtitle = paste0(config$k_cv, "-fold cross-validation (repeated x", 
                     config$times_cv, "), measures averaged per fold")) + 
    ...
    gg <- ggplot(df_plot_main_disease, aes(x = disease, y = value)) + 
      ...
      ggtitle(paste0("Performance using ", input, "-related data as input and the ", network, " network"), 
              subtitle = paste0(config$k_cv, "-fold cross-validation (repeated x", 
                     config$times_cv, "), measures averaged per fold (excluding ", 
                     methods_excluded, ")")) + 
      theme(aspect.ratio = 1, 
...
gg <- ggplot(df_modularity_join, 
  ...
  theme(aspect.ratio = 1) + 
  xlim(-2.5, 24) +
  xlab("Overall performance ranking (drugs input, top 20 hits). Lower is better.") + 
  ylab("Modularity ranking (STRING network). Lower is better.") + 
  ggtitle("Disease performance in terms of modularity and gene list size", 
...
writeLines(out, con = paste0(config$dir_metadata, "/33_sessionInfo.txt"))
```
