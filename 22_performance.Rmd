...
bag_svm <- function(ind_train, ind_test, K, ytrain, B = 30, seeds, ...) {
...
mlr_svm_rf <- function(ind_train, ind_test, df_features, ytrain, seeds) {
        ...
        function(df_in) {
          ...
          list_perf <- plyr::l_ply(
            ...
            function(split_cv_name) {
              # browser()
              # Increase in 1 the seed. Make sure we are not overflowing: modulus a big number
              seed.index <<- as.integer((seed.index + 1L) %% 100000L)
              message("Current seed: ", seed.index)
              ...
              # diffuStats
              # set.seed(seed.index) # probably not necessary
              ...
              parallelStartMulticore(config$nslaves)
              # set.seed(seed.index, "L'Ecuyer") # necessary
              ...
              # It is important to leave it last because it can break down 
              # the reproducibility of the other methods..!
              ...
              # if (seed.index == 3) stop("Stop here now!")
            }
          )
        }
      )
